# 질문
1. 성공테스트 클래스, 실패테스트클래스를 따로 관리하는게 좋은것인지?
...
- 동기화 고민> 동기화테스트는 통합테스트에서 구현>실패하는걸 확인하고 green이 되는 조건으로 syncronized로 처리>추후리팩토링(고민을 별로 하지않았음)
...
- 동시성테스트하려고 ExecutorService의 스레드 풀을 작성하였는데 테스트코드작성시 스레드풀은 몇으로 테스트해야 되나??
...
- 테스트코드를 작성하면서 드는생각들
    - 컨벤션,주석을 한번 스스로 정의내리면서 하였는데 이게 편한정작 편한방법인가 생각이 들었음(보통 어떤걸 약속하지?)
